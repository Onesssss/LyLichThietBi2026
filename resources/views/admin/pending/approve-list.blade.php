...
<main class="main-content">
    <div class="top-bar">
        <div class="page-title">
            <h1><i class="fas fa-check-circle"></i> Duyệt yêu cầu - Danh sách thiết bị</h1>
        </div>
    ...
    <div class="info-note" style="margin-bottom: 20px;">
        ...
        <span>Xem chi tiết yêu cầu thêm/sửa/xóa danh sách thiết bị và thực hiện duyệt hoặc từ chối.</span>
    </div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 380px; gap: 25px;">
        ...
        <div style="background: white; border-radius: 12px; padding: 25px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
            ...
            <div style="margin-bottom: 30px;">
                ...
                <div style="display: grid; grid-template-columns: 160px 1fr; gap: 12px;">
                    {{-- <div style="font-weight: 500; color: #6c757d;">Mã danh sách:</div>
                    <div><strong>{{ $pending->code }}</strong></div> --}}

                    <div style="font-weight: 500; color: #6c757d;">Tên danh sách:</div>
                    <div><strong>{{ $pending->name }}</strong></div>

                    <div style="font-weight: 500; color: #6c757d;">Chốt ID:</div>
                    <div>{{ $pending->point_id }}</div>

                    <div style="font-weight: 500; color: #6c757d;">Mô tả:</div>
                    <div style="background: #f8f9fa; padding: 8px 12px; border-radius: 6px;">{{ $pending->description ?? '---' }}</div>
            ...
            <div>
                ...
                <div style="background: #fff3cd; border-radius: 8px; padding: 15px;">
                    ...
                    <div style="display: grid; grid-template-columns: 160px 1fr; gap: 12px;">
                        {{-- <div style="font-weight: 500; color: #856404;">Mã danh sách:</div>
                        <div>{{ $oldData['code'] ?? 'N/A' }}</div> --}}

                        <div style="font-weight: 500; color: #856404;">Tên danh sách:</div>
                        <div>{{ $oldData['name'] ?? 'N/A' }}</div>

                        <div style="font-weight: 500; color: #856404;">Chốt ID:</div>
                        <div>{{ $oldData['point_id'] ?? 'N/A' }}</div>

                        <div style="font-weight: 500; color: #856404;">Trạng thái:</div>
                        <div>
                            @if(($oldData['status'] ?? 0) == 1)
                                Hoạt động
                            @else
                                Vô hiệu
                            @endif
        ...
        <div style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%); border-radius: 12px; padding: 20px; color: white;">
                ...
                <form action="{{ route('admin.pending.approve-list', $pending->id) }}" method="POST">
                    @csrf
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                ...
                <form action="{{ route('admin.pending.reject-list', $pending->id) }}" method="POST">
                    @csrf
