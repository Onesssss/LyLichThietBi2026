...
<main class="main-content">
    <div class="top-bar">
        <div class="page-title">
            <h1><i class="fas fa-check-circle"></i> Duyệt yêu cầu - Thiết bị</h1>
        </div>
    ...
    <div class="info-note" style="margin-bottom: 20px;">
        ...
        <span>Xem chi tiết yêu cầu thêm/sửa/xóa thiết bị và thực hiện duyệt hoặc từ chối.</span>
    </div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 380px; gap: 25px;">
        ...
        <div style="background: white; border-radius: 12px; padding: 25px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
            ...
            <div style="margin-bottom: 30px;">
                ...
                <div style="display: grid; grid-template-columns: 160px 1fr; gap: 12px;">
                    {{-- <div style="font-weight: 500; color: #6c757d;">Mã thiết bị:</div>
                    <div><strong>{{ $pending->code }}</strong></div> --}}

                    <div style="font-weight: 500; color: #6c757d;">Tên thiết bị:</div>
                    <div><strong>{{ $pending->name }}</strong></div>

                    <div style="font-weight: 500; color: #6c757d;">Danh mục ID:</div>
                    <div>{{ $pending->category_id }}</div>

                    <div style="font-weight: 500; color: #6c757d;">Chốt ID:</div>
                    <div>{{ $pending->point_id }}</div>

                    <div style="font-weight: 500; color: #6c757d;">Vật liệu:</div>
                    <div>{{ $pending->material ?? '---' }}</div>

                    <div style="font-weight: 500; color: #6c757d;">Đơn vị:</div>
                    <div>{{ $pending->unit ?? '---' }}</div>

                    <div style="font-weight: 500; color: #6c757d;">Số lượng:</div>
                    <div><span style="font-size: 18px; font-weight: bold; color: #667eea;">{{ number_format($pending->quantity) }}</span></div>

                    <div style="font-weight: 500; color: #6c757d;">Năm sản xuất:</div>
                    <div>{{ $pending->manufacture_year ?? '---' }}</div>

                    <div style="font-weight: 500; color: #6c757d;">Hạn dùng:</div>
                    <div>{{ $pending->expiry_date ? date('d/m/Y', strtotime($pending->expiry_date)) : '---' }}</div>

                    <div style="font-weight: 500; color: #6c757d;">Tình trạng:</div>
                    <div>
                        @if($pending->condition == 1)
                            <span style="background: #28a745; color: white; padding: 4px 12px; border-radius: 20px; font-size: 13px;">🟢 Tốt</span>
                        @elseif($pending->condition == 2)
                            <span style="background: #ffc107; color: #212529; padding: 4px 12px; border-radius: 20px; font-size: 13px;">🟡 Trung bình</span>
                        @else
                            <span style="background: #dc3545; color: white; padding: 4px 12px; border-radius: 20px; font-size: 13px;">🔴 Hỏng</span>
                        @endif
                    </div>
            ...
            <div style="margin-top: 30px;">
                ...
                <div style="background: #fff3cd; border-radius: 8px; padding: 15px;">
                    @php $oldData = json_decode($pending->original_data, true); @endphp
                    <div style="display: grid; grid-template-columns: 160px 1fr; gap: 12px;">
                        {{-- <div style="font-weight: 500; color: #856404;">Mã thiết bị:</div>
                        <div>{{ $oldData['code'] ?? 'N/A' }}</div> --}}

                        <div style="font-weight: 500; color: #856404;">Tên thiết bị:</div>
                        <div>{{ $oldData['name'] ?? 'N/A' }}</div>

                        <div style="font-weight: 500; color: #856404;">Số lượng:</div>
                        <div>{{ number_format($oldData['quantity'] ?? 0) }}</div>

                        <div style="font-weight: 500; color: #856404;">Tình trạng:</div>
                        <div>
                            @if(($oldData['condition'] ?? 0) == 1)
                                Tốt
                            @elseif(($oldData['condition'] ?? 0) == 2)
                                Trung bình
                            @else
                                Hỏng
                            @endif
        ...
        <div style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%); border-radius: 12px; padding: 20px; color: white;">
                ...
                <form action="{{ route('admin.pending.approve-item', $pending->id) }}" method="POST">
                    @csrf
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                ...
                <form action="{{ route('admin.pending.reject-item', $pending->id) }}" method="POST">
                    @csrf
